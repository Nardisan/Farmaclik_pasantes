...
#REPORTE DE GANANCIA Y PERDIDA PARA TODAS LAS EMPRESAS 
#RERENCIA A LAS EMPRESAS DE FORMA CORRECTA (self.env.company)
...
class AccountingReportWizard(models.TransientModel):
    ...
    def print_xls(self):
        ...
        datas = {'account_report_id': [self.account_report_id.id, 'Ganancia y Perdida'],
                 'company_id': [self.env.company.id, self.env.company.name], 'comparison_context': comparison_context,
        ...
        worksheet.write_merge(3, 3, 0, 3,
                              'Balance Sheet' if self._context.get('from_balance_sheet') else 'Ganancia y perdida',
                              style=styleBorder)
        worksheet.write_merge(4, 4, 0, 3,
                              'Movimientos de destino: ' + 'Todas las entradas ' if self.target_move == 'all' else 'Movimientos de destino: Todas las entradas publicadas')

        if self.date_from and self.date_to:
            worksheet.write_merge(5, 5, 0, 3,
                                  'Desde : ' + str(self.date_from) + ' ' + 'Hasta: ' + str(self.date_to))
        row = 8
        if self.debit_credit:
            fontbold.font = fontP
            worksheet.write_merge(7, 7, 0, 0, 'Nombre', stylePC)
            worksheet.write_merge(7, 7, 1, 1, 'Debito', stylePC)
            worksheet.write_merge(7, 7, 2, 2, 'Credito', stylePC)
            worksheet.write_merge(7, 7, 3, 3, 'Equilibrio', fontbold)
            for each in lines:
                fontbold.font = fontN
        ...
        if not self.enable_filter and not self.debit_credit:
            fontbold.font = fontP
            worksheet.write_merge(7, 7, 0, 0, 'Nombre', stylePC)
            worksheet.write_merge(7, 7, 1, 1, 'Equilibrio', fontbold)
            for each in lines:
                fontbold.font = fontN
                normal_style.font = fontN
                if not each.get('level') > 3:
        ...
        if self.enable_filter and not self.debit_credit:
            fontbold.font = fontP
            worksheet.write_merge(7, 7, 0, 0, 'Nombre', stylePC)
            worksheet.write_merge(7, 7, 1, 1, 'Debito', fontbold)
